...
//---------------------------------------------------------
// Procedure: setMaxRudder(string)

bool Thruster::setMaxRudder(string max_rud)
{
  if(!isNumber(max_rud)) {
    addWarning("MAX_RUDDER is not a number. Check .moos file.");
    return(false);
  }

  return(setMaxRudder(atof(max_rud.c_str())));
}

//---------------------------------------------------------
// Procedure: setMaxRudder(double)
...
bool Thruster::setMaxRudder(double max_rud)
{
  if((max_rud > 0) && (max_rud <= 180)) {
    ...
    string msg = "MAX_RUDDER out of range (0,180].  Check .MOOS file.";
    addWarning(msg);
    return(false);
  }
}
